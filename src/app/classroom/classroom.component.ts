...
export class ClassroomComponent implements OnInit {
  ...
  submitForm(e: Event) {
    e.preventDefault();
    this.isLoading = true;
    const docName = this.validateForm.value.classroomName;
    const docShortName = this.validateForm.value.classroomShortName;
    this.http
      .post('/class', { name: docName, short_name: docShortName })
      .pipe(
        catchError((err) => {
          return of();
        })
