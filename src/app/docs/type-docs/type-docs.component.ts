...
@Component({
  ...
  templateUrl: './type-docs.component.html',
  styleUrls: ['./type-docs.component.less'],
})
export class TypeDocsComponent implements OnInit {
  ...
  submitUpdate(e: Event) {
    ...
    this.isLoading = true;
    this.http
      .patch(`/document-type/${this.tableId}`, {
  ...
  submitForm(e: Event) {
    ...
    this.http
      .post('/document-type', { name: docName })
      .pipe(
        catchError((err) => {
          return of();
        })
